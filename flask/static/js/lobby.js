    ...
    rooms.forEach((room, i) => {
        ...
        card.onclick = () => {
            if (room.configured && room.streamType === 'mic') {
                ...
                window.location.href = `/stream/${room.tenant_id}?url=${encodeURIComponent(room.videoUrl)}&type=${room.streamType || 'url'}`;
            } else {
...
function openRoom(event, tenant_id) {
    ...
    if (room && room.configured && room.streamType === 'mic') {
        ...
        window.location.href = `/stream/${tenant_id}?url=${encodeURIComponent(room.videoUrl)}&type=${room.streamType || 'url'}`;
    } else {
