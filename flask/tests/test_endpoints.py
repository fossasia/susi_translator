"""
Integration tests exercising the Flask routes via the test_client.

These deliberately do NOT spawn the worker thread; we manually stage data in
``transcriptd`` to test read endpoints, and we verify that ``POST /transcribe``
enqueues a payload (without waiting for it to be processed).
"""
...
def test_transcribe_enqueues_and_returns_processing(client, ts):
    payload = {
        "audio_b64": "AAAA",  # any non-empty string
        "chunk_id": "12345",
        "tenant_id": "tenant-x",
    }
    ...
    # The worker thread isn't running in tests, so the item should still be
...
def test_transcribe_rejects_empty_payload(client):
    ...
    # An empty body parses to None; the route returns 400 either way.
    assert resp.status_code in (400, 415)


# ---------------------------------------------------------------------------
# /list_transcripts and /get_transcript
# ---------------------------------------------------------------------------

def _seed(ts, tenant_id: str, items: dict):
    """Helper: populate transcriptd directly under the shared lock."""
...
def test_list_transcripts_rejects_non_integer_from(client, ts):
    """Regression: previously a non-int `from` would propagate ValueError
    and become a 500."""
    _seed(ts, "t1", {"100": "a"})
...
def test_get_transcript_returns_empty_when_no_session(client):
    """Regression: known source with no registered session must NOT 500."""
...
def test_get_transcript_rejects_unknown_source(client):
    """Regression: typo'd source must surface as HTTP 400, not 500."""
    resp = client.get("/get_transcript?source=microphone")  # not in VALID_SOURCES
    assert resp.status_code == 400
...
# ---------------------------------------------------------------------------
# Swagger sanity: /transcribe ack model is registered as 'TranscribeAck'
# ---------------------------------------------------------------------------

def test_swagger_has_distinct_models(client):
    """Regression: previously both api.model() calls used name 'Transcript',
    so Swagger silently overwrote one. Make sure the rename stuck."""
